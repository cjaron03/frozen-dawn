...
final class MonitoringTerminalLayout {
    ...
    private MonitoringTerminalLayout(int panelX, int panelY, int panelW, int panelH,
    ...
    static MonitoringTerminalLayout create(int width, int height, Font font, MonitoringTerminalViewModel viewModel) {
        ...
        int segmentWidth = MonitoringTerminalPuzzle.SEGMENT_LENGTH * charStep;
        ...
        return new MonitoringTerminalLayout(panelX, panelY, panelW, panelH, titleY, attemptsY, headerBottomY,
    ...
    int pageTotal(int archivePageCount) {
        return archivePageCount > 0 ? archivePageCount : MonitoringStationArchive.PAGE_COUNT;
    }
    ...
    private static int desiredAuditHeight(Font font, float auditScale, int auditLineHeight, int contentWidth,
        ...
        int visibleLines = Math.max(viewModel.state() == OpenMonitoringTerminalPayload.STATE_ACTIVE ? 4 : 3,
                Math.min(8, wrappedLines + (viewModel.state() == OpenMonitoringTerminalPayload.STATE_ACTIVE ? 1 : 0)));
    ...
    private static List<TokenHitbox> buildHitboxes(MonitoringTerminalViewModel viewModel, int boardCenterX, int boardY,
        ...
        for (MonitoringTerminalPuzzle.WordToken token : viewModel.board().wordTokens()) {
        ...
        for (MonitoringTerminalPuzzle.PairToken token : viewModel.board().pairTokens()) {
