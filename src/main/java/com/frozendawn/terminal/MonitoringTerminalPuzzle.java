...
public final class MonitoringTerminalPuzzle {
    ...
    private static final String[] WORDS_5 = {
            "CLOUD", "FROST", "STORM", "GAUGE", "RADAR", "CHART", "FRONT", "WINDS",
            "RAINS", "DRIFT", "POLAR", "CLEAR", "PROBE", "GALES", "SLEET", "TRACK",
            "PHASE", "SNOWS"
    };

    private MonitoringTerminalPuzzle() {
    }

    public static Board create(long nonce) {
        Random random = new Random(nonce ^ 0x51A71051A710L);
        List<String> pool = List.of(WORDS_5);
        String password = pool.get(random.nextInt(pool.size()));
        List<String> candidates = selectCandidates(pool, password, random);
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            int start = random.nextInt(SEGMENT_LENGTH - 5 + 1);
            writeToken(segments[segmentIndex], word, start);
            wordTokens.add(new WordToken(segmentIndex, start, 5, i));
        }
        ...
        for (int i = 0; i < PAIR_COUNT && !segmentOrder.isEmpty(); i++) {
            int segmentIndex = segmentOrder.remove(segmentOrder.size() - 1);
            int pairLen = 4 + random.nextInt(3);
            ...
            pairTokens.add(new PairToken(segmentIndex, start, pairLen, i, rewards.get(i % rewards.size())));
        }
        ...
        return new Board(5, renderedSegments, candidates, wordTokens, pairTokens, passwordIndex);
    }
    ...
    private static List<String> selectCandidates(List<String> pool, String password, Random random) {
        ...
        List<Integer> likenessOrder = List.of(5, 0, 4, 1, 3, 2);
