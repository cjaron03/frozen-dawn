...
Spatial approach has a 240-tick budget. The local executor uses loaded, level, walkable cells within 24 blocks of its goal, evaluates at most 80 D* nodes per tick, and considers at most eight remembered nearby danger points. Walking within three blocks of a danger point adds cost, encouraging a short detour. These positioning routes do not build, breach, climb or jump. If the exact destination is unreachable, a local breadth-first search seeks a reachable inspection vantage within four blocks of the recorded crossing. It visits at most 80 cells per tick, retains at most 2,048 cells, stays within the same 24-block radius and 240-tick approach deadline, and reads only loaded, level, walkable cells. Choosing a vantage does not change memory: discovery still requires the actual Architect to arrive and see the obstruction. If no safe inspection route exists, execution releases to the normal Architect. This conservative first implementation can fall back on uneven ground. Once reached, the point is held for the existing 400 ticks with the thinking cue and close-range defense.
...
### Review follow-up: unreachable remembered destination

A post-readiness review identified a case not exercised by the accepted live replay: sealing the remembered outside cell itself, or starting across an impassable wall, could make the exact D* goal fail before the actor reached discovery range. The local inspection fallback fixes that case without changing the observation or confidence rules. Two required GameTests now drive actual entity movement from beyond discovery range in both layouts and verify visible obstruction evidence, replacement knowledge, cooldown, intact walls and no chunk loading. The accepted live replay above remains evidence for the original open-goal layout; these added layouts have automated coverage.

Review-fix gates: `architectVerify architectMonkey` passed with 572 unit tests, 84 regression GameTests (79 required), 584 stress-run GameTests (500 required seeded cases), and eight gate-harness tests. Existing fixtures and seeds were retained.
