...
| Bow cover felt arbitrary; stationary peeking stalled pressure. | Historical ranged confidence at least 0.75 strengthens ordinary cover preference. Two local torso sight rays guide a grounded wall. Repeated fortification requires at least 120 ticks and two blocks of movement; the existing six-block pool bounds construction. Ordinary PEEK ends after 30 ticks and has a 100-tick retry gap plus movement. | Cover reacts to locally visible geometry, not bow-draw input or future arrows. Actual damage interrupts a thinking hold; misses do not. Positional commitments retain their 400-tick duration. |
...
Required command: `./gradlew architectVerify architectMonkey --console=plain`, using Java 21 and the existing seeds and shared fixtures. The accepted source passed 609 unit tests, 142 native GameTests including all 137 required entries, and 500 seeded stress cases; the stress server also repeated all 142 native tests. There were no failures, errors or skips. The PR publishes fresh checks for its exact head; the tracked record identifies the source and artifact used for live acceptance.
...
- Reproduce round 9's four `inWall` damage events during retreat. Their cause remains unresolved, and later rounds did not reproduce them.
- Review round 10's pursuit removing the upper block of its own recent cover. The journal establishes the interaction, not whether the wall's net tactical value was negative.
