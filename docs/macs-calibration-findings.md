...
| Snow cover worked, but three-block pillars looked wasteful and pursuit could circle. | The owner accepted two-block pillars and the twelve-block pool, then accepted the later pursuit fixes. Recovery handles circles, and stalled raised goals refresh after the target moves away. | Acceptance covers the recorded fixtures and live routes, not zero possible stalls. Retreat construction logic is unchanged. See the [guard acceptance](macs-guard-acceptance.md). |
| Active sword guard repeatedly canceled valid snow mining, and a canceled breach could discard its accepted walking route. | Lowered guarding now permits queued mining. Accepted walking paths retain ownership until completed, replaced or invalidated by target movement. Required native cases fail before the fixes; the accepted live round completed nine breaks and five walking cancellations. | The final round used Normal and one player. Multiplayer remains deferred. |
...
The latest solo acceptance is recorded in [guard acceptance](macs-guard-acceptance.md) and its [verification summary](verification/macs-guard-acceptance.json). It covers natural sword victory, both axe contact windows, post-heal guard behavior, learned carryover and the three pursuit repairs. The owner accepted guard and snow pursuit after the final 21:35 replay. That replay's peak no-progress count was 45 and the new elevated-goal refresh actually fired. Earlier dated presentation replays remain below.
...
The latest guard/terrain build passed 609 unit tests, eight gate-harness tests, 162 native GameTests (all 157 required), and all 500 unchanged seeded stress cases; the stress server repeated all 162 native cases. Its source and artifact hashes are in the newer guard-acceptance verification summary. The older calibration record remains the provenance for the original sustained-shield replay.
